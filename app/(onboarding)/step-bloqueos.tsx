...
export default function StepBloqueos() {
  ...
  function handleToggleApp(app: { package: string; nombre: string }) {
    const existe = data.apps_bloquear.find((b) => b.app_package === app.package);
    if (existe) {
      removeBloqueo(app.package);
    } else {
    ...
    >
      ...
      <View className="gap-2 mt-6">
        {APPS_POPULARES.map((app) => {
          const selected = data.apps_bloquear.some((b) => b.app_package === app.package);
          return (
            <Pressable
              key={app.package}
