  ...
  return (
    ...
    >
      ...
      <View className="mt-4 gap-3">
        {ejercicios.map((ej: Record<string, unknown>, idx: number) => (
          <EjercicioCard
            key={`${ej.ejercicio_id || idx}-${idx}`}
            ejercicio={ej as any}
            index={idx}
          />
        ))}
      ...
      <Button
        ...
        loading={loading}
        onPress={() => {
          const planEjercicios = ejercicios.map((ej: Record<string, unknown>) => ({
            ejercicio_id: String(ej.ejercicio_id || ej.nombre || ''),
            nombre: String(ej.nombre || ''),
            series: Number(ej.series) || 3,
            reps: String(ej.reps || '10'),
            descanso_seg: Number(ej.descanso_seg) || 60,
            peso_kg: ej.peso_kg != null ? Number(ej.peso_kg) : null,
            nota: ej.nota ? String(ej.nota) : undefined,
            variante_casa: ej.variante_casa || null,
          }));
          iniciarWorkout(
