...
export default function ConsultaIAModal() {
  const { consulta, loadingState } = useForgeAI();
  ...
  async function handleEnviar() {
    ...
    if (!q || loadingState.isLoading) return;
    ...
    if (result?.success && result.data) {
      ...
      setMensajes((prev) => [
        ...
        {
          role: 'forge',
          content: (data.respuesta as string) || 'Sin respuesta.',
          acciones: (data.acciones_sugeridas as string[]) || [],
        },
      ]);
    } else {
      setMensajes((prev) => [
    ...
    >
      ...
      <View className="p-4 border-t border-border flex-row gap-2 items-end">
        ...
        <Button
          title="→"
          variant="primary"
          disabled={!pregunta.trim() || loadingState.isLoading}
