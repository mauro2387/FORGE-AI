...
export default function CheckinModal() {
  const { checkin: enviarCheckin, loadingState } = useForgeAI();
  ...
  async function handleSubmit() {
    ...
    const result = await enviarCheckin(checkinData);
    if (result?.success && result.data) {
      setRespuestaIA(result.data as Record<string, unknown>);
    }
  }
      ...
      >
        ...
        <Card className="mt-6">
          <Text style={{ fontFamily: FONTS.body, fontSize: 15, color: COLORS.text, lineHeight: 22 }}>
            {(respuestaIA.mensaje as string) || 'Check-in procesado.'}
          </Text>
        </Card>
        ...
        {respuestaIA.evaluacion_dia && (
          <Card className="mt-4">
            ...
            >
              {(respuestaIA.evaluacion_dia as Record<string, unknown>).nota}/10
            </Text>
          </Card>
        )}
        ...
        {respuestaIA.consejo_manana && (
          <Card className="mt-4">
            ...
            <Text style={{ fontFamily: FONTS.body, fontSize: 13, color: COLORS.text, marginTop: 4 }}>
              {respuestaIA.consejo_manana as string}
            </Text>
          </Card>
        )}
